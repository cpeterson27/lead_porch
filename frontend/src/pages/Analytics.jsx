...
export default function Analytics() {
  ...
  return <div className="page-dashboard analytics-page"><div className="page-header"><div><p className="page-eyebrow">Growth intelligence</p><h1 className="page-title">Analytics</h1><p className="page-subtitle">Sales, marketing, coaching, revenue, communications, referrals, and social attribution from canonical records.</p></div><span className="analytics-health is-healthy">Canonical data · {new Date(data.generatedAt).toLocaleString()}</span></div>
    ...
    <h2>Revenue</h2>
    <section className="analytics-metrics"><DashboardCard title="Contracted revenue"><strong>{currency(data.revenue.contractedRevenue)}</strong><span>full value of every closed-won deal</span></DashboardCard><DashboardCard title="Cash collected"><strong>{currency(data.revenue.cashCollected)}</strong><span>what has actually been paid</span></DashboardCard><DashboardCard title="Outstanding balance"><strong>{currency(data.revenue.outstandingBalance)}</strong><span>contracted minus collected</span></DashboardCard><DashboardCard title={`${data.revenue.target?.year || ""} YTD vs. target`}><strong>{currency(data.revenue.ytd)}</strong><span>{data.revenue.target?.progressPercent ?? 0}% of {currency(data.revenue.target?.amount)}</span></DashboardCard></section>
    <section className="analytics-metrics"><DashboardCard title="Program revenue"><strong>{currency(data.revenue.programRevenue)}</strong></DashboardCard><DashboardCard title="Add-on revenue"><strong>{currency(data.revenue.addonRevenue)}</strong></DashboardCard><DashboardCard title="Referral-generated"><strong>{currency(data.revenue.referralGenerated)}</strong></DashboardCard><DashboardCard title="Commission expense"><strong>{currency(data.revenue.commissionExpense)}</strong></DashboardCard><DashboardCard title="Customer lifetime value"><strong>{currency(data.revenue.customerLifetimeValue)}</strong></DashboardCard></section>
    <MetricTable title="Revenue by month" rows={data.revenue.monthly || []} columns={[{key:"month",label:"Month"},{key:"deals",label:"Deals won"},{key:"contracted",label:"Contracted",format:currency},{key:"cashCollected",label:"Cash collected",format:currency}]} />
    ...
    <p className="analytics-disclaimer">Attribution includes only identifiable canonical Contacts and supported events. The AI search traffic panel separately reports anonymous AI referral visits. Other anonymous views, likes, saves, and unsupported Skool course progress are excluded.</p>
  </div>;
}
