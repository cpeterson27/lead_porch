        ...
        {run.status === "draft" ? (
          <>
            ...
            <div className="leadgen-review-grid">
              ...
              <label><span>Web search cash cap ($) — Vertex + OpenAI only</span><input type="number" min="0" max="1000" step="0.5" disabled={!sources.length} title={sources.length ? undefined : "Vertex and OpenAI Web Search are both off for this run — this field does nothing until one is on."} value={run.providerCreditCapUsd} onChange={(event) => updateRunField(run, "providerCreditCapUsd", Number(event.target.value))} /></label>
              <label><span>Page limit per query — Vertex + OpenAI only</span><input type="number" min="1" max="10" disabled={!sources.length} title={sources.length ? undefined : "Vertex and OpenAI Web Search are both off for this run — this field does nothing until one is on."} value={run.pageLimitPerQuery} onChange={(event) => updateRunField(run, "pageLimitPerQuery", Number(event.target.value))} /></label>
              <label><span>Query limit per run (web queries only — never PDL or Apollo)</span><input type="number" min="1" max="500" disabled={!sources.length} title={sources.length ? undefined : "Vertex and OpenAI Web Search are both off for this run — this field does nothing until one is on."} value={run.queryLimitPerRun} onChange={(event) => updateRunField(run, "queryLimitPerRun", Number(event.target.value))} /></label>
              <label><span>Retry attempts per query — Vertex + OpenAI only</span><input type="number" min="1" max="10" disabled={!sources.length} title={sources.length ? undefined : "Vertex and OpenAI Web Search are both off for this run — this field does nothing until one is on."} value={run.retryPolicy?.maxAttemptsPerJob || 3} onChange={(event) => updateRunInState({ ...run, retryPolicy: { maxAttemptsPerJob: Number(event.target.value) } })} /></label>
            </div>
