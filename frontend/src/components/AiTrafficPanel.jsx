...
export default function AiTrafficPanel() {
  ...
  return <section id="ai-search-traffic" className="ai-traffic-panel" aria-labelledby="ai-traffic-title" aria-busy={loading}>
    ...
    {!data ? <p role="status">{loading ? "Loading AI search traffic…" : "No report is available yet."}</p> : <>
      ...
      {totals.visits === 0 ? <div className="ai-traffic-empty"><strong>Ready to collect AI referrals</strong><p>No identifiable AI visits in this period yet. New visits appear here when a source tag or referring website identifies an AI tool. Historical visits are not backfilled.</p></div> : null}
      ...
      <p className="ai-traffic-note">This measures identifiable referral traffic, not AI mentions or rankings. Google AI Overviews, Bing AI answers, and visits without source information cannot be reliably separated from ordinary search or direct traffic. Visits use anonymous sessions that expire after 30 minutes of inactivity; counts are not unique people. Tracking respects browser privacy signals. Records are retained for 90 days.</p>
    </>}
  </section>;
}
