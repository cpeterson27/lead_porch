...
router.post("/chat", async (req, res) => {
  try {
    ...
    if (recurringLeadRequest) {
      ...
      const schedule = await DiscoverySchedule.findOne({ workspaceId: req.auth.workspaceId, programNoteId: program.noteId, name: scheduleName }) || await DiscoverySchedule.create({ workspaceId: req.auth.workspaceId, name: scheduleName, programNoteId: program.noteId, programName: program.title, enabled: false, intervalMinutes: 1440, dailyCandidateTarget: target, pageLimitPerQuery: 1, queryLimitPerRun: Math.max(20, target), providerCreditCapUsd: 1, sources: ["vertex", "openai_web_search"], includePdlPersonSearch: true, maxPdlPersonSearchCredits: target, includePdlCrossReference: false, maxPdlCrossReferenceCredits: 0, maxAttemptsPerJob: 2, createdByUserId: req.auth.user?._id });
...
router.post("/morning-leads/prepare", requireRole("owner", "admin"), async (req, res) => {
  try {
    ...
    const schedule = await DiscoverySchedule.create({ workspaceId: req.auth.workspaceId, name: req.body?.name || "100 prospective students every morning", programNoteId: req.body.programNoteId, programName: req.body?.programName || "", enabled: false, intervalMinutes: 1440, dailyCandidateTarget: Math.min(500, Math.max(1, Number(req.body?.dailyCandidateTarget) || 100)), pageLimitPerQuery: Math.min(10, Math.max(1, Number(req.body?.pageLimitPerQuery) || 1)), queryLimitPerRun: Math.min(500, Math.max(1, Number(req.body?.queryLimitPerRun) || 100)), providerCreditCapUsd: Math.min(1000, Math.max(0, Number(req.body?.providerCreditCapUsd) || 1)), sources: Array.isArray(req.body?.sources) ? req.body.sources.filter((s) => ["vertex", "openai_web_search"].includes(s)) : ["vertex", "openai_web_search"], includePdlPersonSearch: req.body?.includePdlPersonSearch !== false, maxPdlPersonSearchCredits: Math.min(500, Math.max(0, Number(req.body?.maxPdlPersonSearchCredits) || 100)), includePdlCrossReference: req.body?.includePdlCrossReference === true, maxPdlCrossReferenceCredits: Math.min(500, Math.max(0, Number(req.body?.maxPdlCrossReferenceCredits) || 0)), maxAttemptsPerJob: 2, createdByUserId: req.auth.user?._id });
...
router.post("/memory/notes/upload-pdfs", requireRole("owner"), (req, res, next) => {
  ...
  for (const file of files) {
    try {
      ...
      results.push({ filename: file.originalname, success: false, error: error.message, code: error.code || "PDF_INGEST_FAILED", existingNoteId: error.existingNoteId || null });
    }
  }
