...
async function getStatus(workspaceId) {
  if (memorySource() === "cloud") {
    ...
    const noteCount = configured && workspaceId ? await JarvisMemoryNote.countDocuments({ workspaceId, source: { $in: ["obsidian_bridge", "approved_memory", "pdf_upload", "conversation_capture"] } }) : 0;
...
async function retrieveCloudNotes(query, { workspaceId, categories, limit = 4 } = {}, Model = JarvisMemoryNote) {
  ...
  const filter = { workspaceId, source: { $in: ["obsidian_bridge", "approved_memory", "pdf_upload", "conversation_capture"] }, status: "approved" };
