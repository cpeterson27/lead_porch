...
async function qualifyAndRecommend({ workspaceId, userId, auth, resultIds, correlationId = "" }, dependencies = {}) {
  ...
  const result = await runAgent({
    ...
    // into this prompt can genuinely exceed the default 20000-character
    // operational-context limit — the resulting blind truncation cut off
    // JSON mid-object for whichever candidates landed past the cutoff, and
